  ...
  return <>
    ...
    <div className="fixed bottom-5 right-5 z-[120]">
      {open && <section className="mb-3 flex h-[min(650px,calc(100vh-100px))] w-[min(410px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        {!selectedId ? <div className="flex min-h-0 flex-1 flex-col"><div className="border-b border-slate-100 px-4 py-3 text-[11px] font-black uppercase tracking-wider text-slate-500">Conversas</div><div className="max-h-44 shrink-0 overflow-y-auto p-2">{conversations.map((conversation) => <button key={conversation.id} onClick={() => selectConversation(conversation.id)} className="w-full rounded-lg px-3 py-3 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50">{conversation.titulo || 'Conversa'}</button>)}{conversations.length === 0 && <p className="px-3 py-5 text-xs text-slate-400">Nenhuma conversa com este tenant.</p>}</div><div className="flex items-center gap-2 border-t border-slate-100 px-4 py-3 text-[11px] font-black uppercase tracking-wider text-slate-500"><Users size={14} /> Pessoas</div><div className="min-h-0 flex-1 overflow-y-auto p-2">{users.map((user) => <button key={user.user_id} disabled={busy} onClick={() => void startConversation(user.user_id)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-slate-600 hover:bg-slate-50"><span title={presence[user.user_id] === 'online' ? 'Online' : presence[user.user_id] === 'idle' ? 'Inativo ha mais de 30 minutos' : 'Offline'} className={`h-2.5 w-2.5 rounded-full ${statusColor(presence[user.user_id])}`} /><span className="min-w-0 flex-1 truncate">{userName(user)}</span><UserPlus size={14} className="text-emerald-600" /></button>)}</div></div> : <>
